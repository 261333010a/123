-------------------------------------------第十一章
--------------------所有操作执行后要提交
create table emp_new as select * from emp
-------------------------添加数据
-----insert into 只能加一条数据
insert into 表名(列名1，列名2，。。。。)
values (值一，值二，。。。。)

create table emp_copy as select * from emp where 1 = 0 ---------用来快速建表，
                                                       ---------表的数据类型一致，
                                                       ---------1=0：表示内容清空
                                                       
--------疯狂插入数据(插入多条数据 去掉values子句)
insert into 表名 (子查询)
ex:insert into emp_copy select * from emp -----------复制文本内容
 

向单列插入疯狂数据
insert into(empno) select empno from emp [where] 
insert into(empno,ename) select empno,ename from emp [where] 


-------------------------修改数据
update table(表名) 
set column(列名1) = value(值1) [, column = value]
[where] ------不加where，修改全表

alter table table(表名)
add(column(列名) 数据类型)
ex:alter table emp_new 
add(dname varchar(40)) ----------添加一个dname列 ，数据类型为char 字符长度为40


------------------------删除数据
delete [from] table(表名)-----from 可有可无
[where] --------不加where，删除表中内容

--------------------------保存点
---savepoint savepointname
---rollback to savepointname 回滚到指定保存点

--------------------------锁
---只能在更新数据时(修改，删除)，才会上锁，其他用户不能更新此数据
---查询时不影响，只有更新数据的用户才能查询修改之后但未提交的信息，
---其他用户只能查询到更新数据前的数据
---事务结束时，才会解锁

 
-------------------练习一
1.向部门表新增一个部门，部门编号为50，部门名称为HR，工作地点为SY。
insert into dept values(50,'HR','SY')

2.向部门表新增一个部门，部门编号为60，部门名称为MARKET。
insert into dept(deptno,dname) values(60,'MARKET')

-------------------练习二
1.向员工表中新增一个员工，员工编号为8888，姓名为BOB，岗位为CLERK，
经理为号7788，入职日期为1985-03-03，薪资3000，奖金和部门为空。
insert into emp values(8888,'BOB','CLERK',7788,
                       to_date('1985-03-03','YYYY-MM-dd'),3000,null,null)

-------------------练习三
1.使用CREATE TABLE emp_back as 
        SELECT * FROM EMP WHERE 1=0，创建emp_back表,拷贝下来即可。

create table emp_back as select * from emp where 1=0

2.把emp表中入职日期大于1982年1月1日之前的员工信息复制到emp_back表中。
insert into emp_back select * from emp 
                                   where hiredate > to_date('1982-01-01','yyyy-mm-dd')
                                   
-------------------练习四
1.修改部门20的员工信息，把82年之后入职的员工入职日期向后调整10天
update emp_new
set hiredate = hiredate + 10
where  deptno = 20
and to_char(hiredate,'yyyy') > 1982  

2.修改奖金为null的员工，奖金设置为0
update emp_new
set comm = 0
where comm is null

3.修改工作地点在NEW YORK或CHICAGO的员工工资，工资增加500
update emp_new
set sal = sal+500
where deptno in (select deptno from dept where loc in ('NEW YORK','CHICAGO'))

------------------练习五
1.在emp表中增加一个列dname, 来存储部门名
alter table emp_new
add(dname varchar(40))

2.使用相关子查询更新dname列为正确的部门名称。update emp_new en
set dname = (select dname from dept where deptno = en.deptno)

-----------------练习六
1.删除经理编号为7566的员工记录
delete emp_new
where mgr = 7566

2.删除工作在NEW YORK的员工记录
delete emp_new
where deptno = (select deptno from dept where loc = 'NEW YORK')

3.删除工资大于所在部门平均工资的员工记录
delete emp_new en
where sal > (select avg(sal) from emp_new where deptno = en.deptno)

-------------------练习七
分析如下语句序列，哪些语句会结束事务？
INSERT…-------------插入数据
UPDATE..------------修改数据
INSERT--------------插入数据
ROLLBACK;------------回滚结束事务，数据不变
DELETE..-------------删除数据
DELETE..-------------删除数据
SELECT..-------------查询表
COMMIT..-------------提交数据，完成结束事务
INSERT..-------------插入数据
INSERT..-------------插入数据
DELETE..-------------删除数据
GRANT..--------------給数据库用户授权，结束事务
INSERT..-------------插入数据
SELECT;--------------查询表

--------------------练习八
1.test表为空表，分析如下语句操作后，最后test表的状态。
INSERT INTO test(id,name) values(1, 'a')；-------插入数据
INSERT INTO test(id,name) values(2, 'b')；-------插入数据
SAVEPOINT s1;------------------------------------设置保存点 s1
INSERT INTO test(id,name) values(3, 'c')；-------插入数据
INSERT INTO test(id,name) values(4, 'd')；-------插入数据
DELETE FROM test WHERE id in (1,3);--------------删除数据
ROLLBACK TO s1;----------------------------------跳转到保存点s1，保存点之后的操作无效
DELETE FROM test WHERE id in (2,4);--------------删除数据
COMMIT;------------------------------------------提交数据，结束事务
ROLLBACK;----------------------------------------回滚数据，前一句commit已经结束事务，无效操作

--------------------练习九
--------------------课后作业
1.使用如下语句，创建学生表student和班级表class
create table students (        --学生表
      xh char(4),--学号
      xm varchar2(10),--姓名
      sex char(2),--性别
      birthday date,--出生日期
      sal number(7,2), --奖学金
      studentcid number(2) --学生班级号
)
Create table classs (   --班级表
      classid number(2), --班级编号
      cname varchar2(20),--班级名称
        ccount  number(3) --班级人数
)
2.基于上述学生表和班级表，完成如下问题
（1）添加三个班级信息为：1，JAVA1班，null------------添加到班级表 insert
                         2，JAVA2班，null
                         3，JAVA3班，null
insert into classs values(1,'JAVA1班',null)
insert into classs values(2,'JAVA2班',null)
insert into classs values(3,'JAVA3班',null)
                   
（2）添加学生信息如下：‘A001’,‘张三’,‘男’,‘01-5月-05’,100,1------添加到学生表
insert into students values('A001','张三','男','01-5月-05',100,1)

（3）添加学生信息如下：'A002','MIKE','男','1905-05-06',10
insert into students values('A002','MIKE','男',to_date('1905-05-06','yyyy-mm-dd'),10,null)

（4）插入部分学生信息： 'A003','JOHN','女’
insert into students(xh,xm,sex) values('A003','JOHN','女')

（5）将A001学生性别修改为'女‘--------修改信息 update
update students set sex = '女' where xh = 'A001'

（6）将A001学生信息修改如下：性别为男，生日设置为1980-04-01
update students set sex = '男',birthday = to_date('1980-04-01','yyyy-mm-dd')
where xh = 'A001'

（7）将生日为空的学生班级修改为Java3班
update students set studentcid = (select classid from classs where cname = 'JAVA3班')
where birthday is null 


（8）请使用一条SQL语句，使用子查询，更新班级表中每个班级的人数字段
update classs s set ccount  =
                    (select count(*) from students
                            where studentcid = s.classid)
3.使用如下语句，建立以下表
CREATE TABLE copy_emp   (
  empno number(4),----员工编号
  ename varchar2(20),----员工姓名
  hiredate date default sysdate ,-----入职日期
  deptno number(2),-----部门号
  sal number(8,2))-----工资
4.在第三题表的基础上，完成下列问题
(1)在表copy_emp中插入数据，要求sal字段插入空值，部门号50，参加工作时间为2000年1月1日，其他字段随意
insert into copy_emp values(null,null,to_date('2000-01-01','yyyy-mm-dd'),50,null)

(2)在表copy_emp中插入数据，要求把emp表中部门号为10号部门的员工信息插入
insert into copy_emp select empno,ename,hiredate,deptno,sal from emp where deptno = 10

(3)修改copy_emp表中数据，要求10号部门所有员工涨20%的工资
update copy_emp set sal =sal*1.2 where deptno = 10

(4)修改copy_emp表中sal为空的记录，工资修改为平均工资
update copy_emp set sal = (select avg(sal) from copy_emp) where sal is null

(5)把工资为平均工资的员工，工资修改为空
update copy_emp set sal  = null where sal = (select avg(sal) from copy_emp)

(6)另外打开窗口2查看以上修改

(7)执行commit，窗口2中再次查看以上信息
commit;

(8)删除工资为空的员工信息
delete copy_emp where sal is null

(9)执行rollback
rollback;







